...
import type { Method } from '@/types/servers'
...
const changeAnswerSelectedTab = (tab: string) => {
...
const defaultMainSelectedOption = {
  name: 'Автоматический подключен тестовый',
  value: 'hoswbhistory.s2.marketsapi.ru',
};

const paramsMainSelectedOption = ref<paramsMainOption | null>(defaultMainSelectedOption);
watch(paramsMainSelectedOption, (newValue) => {
  if (newValue !== null) {
    console.log('paramsMainSelectedOption ', paramsMainSelectedOption.value)
  }

});

// Задаем значение по умолчанию для Dropdown
const defaultServerOption = {
  name: 'WB - v1 (актуальная)',
  value: 'wb',
};

const serverSelectedOption = ref<ServerOption | null>(defaultServerOption);

watch(serverSelectedOption, (newValue) => {
  if (newValue !== null) {
    serversStore.selectServerByValue(newValue.value); 
  }
...
<template>
  <div class="table desktop">
    <div class="table-version" v-if="serversStore.getNameValueData">
      <CommonDropDown :items="serversStore.getNameData" 
      v-model="serverSelectedOption" 
      :modelValue="defaultServerOption"
      />
    </div>
    ...
    <div class="slider-table">
      ...
      <div class="table-content table-tab-content" v-for="(method, index_method) in serversStore.getMethods"
        ...
        <div class="table-head">
          <span class="table-method" :class="method.type === 'get' ? 'green' : 'blue'">{{ method.name }}</span>
          <span class="text">{{ method.desc }}</span>
      ...
      <div class="table-resizeable-right">
        <div class="table-content">
          <ul class="table-head table-tabs js_tabs">
            <li>
              <button class="btn btn-small btn-grey active" @click="changeAnswerSelectedTab('code')"><span>Примеры
                  кода</span></button>
            </li>
            <li>
              <button class="btn btn-small btn-grey" @click="changeAnswerSelectedTab('response')"><span>Примеры
                  ответа</span></button>
            </li>
            <li>
          ...
          <!-- end .table-head-->
          <div class="table-tab-content" :class="{ visible: selectedAnswerTab === 'code' }"
...
try {
	...
	console.error(error);
          ...
          <!-- end .table-tab-content-->
          <div class="table-tab-content" :class="{ visible: selectedAnswerTab === 'response' }"
...
.custom-select {
  position: relative;
  width: 200px;
  /* Настройте ширину по своему усмотрению */
}


.original-select {
  appearance: none;
  /* Убирает стандартный стиль селекта */
  padding: 8px;
